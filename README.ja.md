...
`dist/ndx.html` を開いて、中身をまるごとブログのHTMLカード（Ghost, WordPress, Hugo等）に貼り付ける。`<ndx-calc>` タグと `<script>` ブロックだけの、自己完結したHTMLスニペットが入っている。

npm publishもCDNリンクも外部ファイルも不要。貼るだけで動く。
...
`dist/index.html` はプレビュー用のフルページとしても生成される。

## 機能

- **シャッタースピード補正** — NDフィルター強度に応じた補正後シャッタースピードを算出
