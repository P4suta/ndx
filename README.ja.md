...
プロジェクトをビルドし、生成された `dist/index.html` の内容をまるごとHTMLページやブログのHTMLカードに貼り付ける。ビルド成果物にHTML・CSS・JSがすべて含まれた単一ファイルになっている。
...
npm publishもCDNリンクも外部スクリプトタグも不要。出力ファイルそのものがコンポーネント。
