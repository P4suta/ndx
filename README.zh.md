...
打开 `dist/ndx.html`，将全部内容复制到博客平台的HTML卡片中（Ghost、WordPress、Hugo等）。文件包含 `<ndx-calc>` 标签和一个内联了所有代码的 `<script>` 块。

无需 npm publish。无需 CDN 链接。无需外部文件。粘贴即可使用。
...
<script>/* ... 压缩后的组件代码 ... */</script>
```

`dist/index.html` 也会同时生成，作为完整的预览页面。

## 功能特性

- **快门速度补偿** — 根据ND滤镜强度计算补偿后的快门速度
